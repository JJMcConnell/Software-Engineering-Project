...
(function (factory) {
    ...
    function Calendar(element, instanceOptions) {
        ...
        function getAndRenderEvents() {
            // HACKED
            // Yeah.. so this hack is pretty sketch... but it makes it so the calendar never reloads events.
            if (false) {// || !options.lazyFetching || isFetchNeeded(currentView.start, currentView.end)) {
                fetchAndRenderEvents();
            }
            else {
                renderEvents();
    ...
    function EventManager(options) { // assumed to be a calendar
        ...
        function mutateEvent(event, newStart, newEnd) {
            ...
            console.log(newStart + moment.duration(oldEnd - oldStart));

            //Made it so that it won't snap to fricken earlier or later than what's actually shown on the stupid calendar.
            //Stupid calendar...
            if (newStart) {
                if (newStart._d.getHours() < 2 || (newStart._d.getHours() == 2 && newStart._d.getMinutes() < 25)) {
                    //newStart.setHours(3);
                    //newStart.setMinutes(25);
                    //event._start._d.setHours(3);
                    //event._start._d.setMinutes(25);
                    newStart = null;
                }
            }
    ...
    */
    ...
    $.extend(TimeGrid.prototype, {
        ...
        slatRowHtml: function () {
            ...
            // The UF period number
            var period = 0;

            var noramlMaxTime = moment.duration('19:20:00');

            // Calculate the time for each slot, upto the last normal (not evening) period
            while (slotTime < noramlMaxTime) {
                ...
                axisHtml =
                    '<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
                        ((!slotNormal || !minutes) ? // if irregular slot duration, or on the hour, then display the time
                            '<span> Period ' + period + ' (' + // for matchCellWidths
            ...
            // Add the evening periods
            for (var i = 1; i <= 3; i++) {
                //Change the slot time to an hour
                hourTime = '01:00:00';
                slotNormal = false;
                ...
                axisHtml =
                    '<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
                        ((!slotNormal || !minutes) ? // if irregular slot duration, or on the hour, then display the time
                            '<span> Period E' + i + ' (' + // for matchCellWidths
                                htmlEscape(calendar.formatDate(slotDate, view.opt('axisFormat'))) + ')' +
                            '</span>' :
                            ''
                            ) +
                    '</td>';
                axisHtml =
                    '<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + 'style = "height: 1.8em" ' + '>' +
                        ((!slotNormal || !minutes) ? // if irregular slot duration, or on the hour, then display the time
                            '<span style="height: 1em"> Period E' + i + ' (' + // for matchCellWidths
                                htmlEscape(calendar.formatDate(slotDate, view.opt('axisFormat'))) + ')' +
                            '</span>' :
                            ''
                            ) +
                ...
                html +=
                    '<tr ' + (!minutes ? '' : 'class="fc-minor"') + '>' +
                        (!isRTL ? axisHtml : '') +
                        '<td class="' + 'fc-evening-slats' + '"' + 'style="height: 1.8em"' + '/>' +
                        (isRTL ? axisHtml : '') +
                    "</tr>";

                // Period time for evening slots is an hour
                slotTime.add(hourTime);
            }
        ...
        processOptions: function () {
            ...
            // HACKED
            // Made it so that it snaps to every 5 minutes
            snapDuration = moment.duration('00:05:00');
        ...
        computeTimeTop: function (time) {
            ...
            var hourDuration = moment.duration('01:00:00');

            // HACKED
            // Evening drag and drop events fixed
            if (moment.duration(time) > moment.duration('19:20:00'))
                slatCoverage = (time - this.minTime - moment.duration('00:55:00')) / hourDuration; // floating-point value of # of slots covered
